...
DRUG_INFO = {
    'cocaine': {
        'class': 'Stimulant',
        'key_peaks': [1705, 1275, 1100, 860, 750, 1450, 2970]   ,
    ...
    'heroin': {
        'class': 'Opioid',
        'key_peaks': [1745, 1245, 1035, 950, 820, 4770, 5254, 9542, 10726],
        'min_peaks_required': 3,
    ...
    'methadone': {
        ...
        'key_peaks': [1715, 1285, 1125, 980, 890, 1450, 3406],
        'min_peaks_required': 3,
    ...
    'meth': {
        'class': 'Stimulant',
        'key_peaks': [2960, 2925, 1490, 1380, 1340, 1040, 665],
        'min_peaks_required': 3,
...
def analyze_spectrum(df, drug_type):
    ...
    # Find peaks
    peaks, _ = find_peaks(df['absorbance'], height=df['absorbance'].quantile(0.80), 
                         prominence=max(0.002, df['absorbance'].std()), distance=10)
...
def extract_features(df):
    ...
    peaks, properties = find_peaks(df['absorbance'], 
                                 height=df['absorbance'].quantile(0.8),
                                 prominence=0.05,
                                 width=5)
    ...
    for drug, info in DRUG_INFO.items():
        ...
        for target_peak in info['key_peaks']:
            ...
            if (df['absorbance'].iloc[idx] > df['absorbance'].quantile(0.7)) and \
               (abs(actual_peak - target_peak) < 10):
                matched_peaks += 1
...
def prepare_training_data(drug_files, non_drug_files):
    """Create enhanced training dataset with noise & temp distortions"""
    samples = []
    ...
    for drug, path in drug_files.items():
        try:
            ...
            for _ in range(5):  # More diverse augmentations
                noisy_df = add_realistic_environmental_noise(df)
                temp = np.random.uniform(20, 40)
                noisy_df['absorbance'] = adjust_absorbance(noisy_df['absorbance'], temp)

                features = extract_features(noisy_df)
    ...
    for i, path in enumerate(non_drug_files):
        try:
            ...
            for _ in range(10):  # More non-drug samples for balance
                noisy_df = add_realistic_environmental_noise(df)
                temp = np.random.uniform(20, 40)
                noisy_df['absorbance'] = adjust_absorbance(noisy_df['absorbance'], temp)
...
def predict_sample(file_path, binary_model, multiclass_model, le):
    ...
    try:
        ...
        if 'wavelength' in df.columns and 'wavenumber' not in df.columns:
        ...
        # Define expected features (fallback if model doesn't provide them)
        expected_features = [
        ...
        # Try to get features from model if available
        try:
            model_features = binary_model.named_steps['xgb'].get_booster().feature_names
            if model_features is not None:
                expected_features = model_features
        except Exception as e:
            print(f"⚠ Couldn't get features from model, using fallback: {str(e)}")
        ...
        if is_drug:
            ...
            for peak in expected_peaks:
                ...
                status = '✅' if closest and abs(closest - peak) < 10 else '❌'
...
if __name__ == "__main__":
    # Define input files
    drug_files = {
        'cocaine': "Pure Model/cocaine.csv",
        'heroin': "Pure Model/heroin.csv",
        'methadone': "Pure Model/methadone.csv",
        'morphine': "Pure Model/morphine.csv",
        'meth' : "Pure Model/meth.csv"
    }

    non_drug_files = [
        "Pure Model/sucrose.csv", 
        "Pure Model/lactic.csv", 
        "Pure Model/citric.csv", 
        "Pure Model/urea.csv",
        "Pure Model/ethanol.csv"
        ]
