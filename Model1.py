...
DRUG_INFO = {
    'cocaine': {
        'class': 'Stimulant',
        'key_peaks': [1705, 1275, 1100, 860, 750],
    ...
    'heroin': {
        'class': 'Opioid',
        'key_peaks': [1745, 1245, 1035, 950, 820],
        'min_peaks_required': 3,
    ...
    'methadone': {
        ...
        'key_peaks': [1715, 1285, 1125, 980, 890],
        'min_peaks_required': 3,
...
def analyze_spectrum(df, drug_type):
    """Enhanced spectrum analysis with visualization"""
    ...
    # Find and plot peaks
    peaks, _ = find_peaks(df['absorbance'], height=df['absorbance'].quantile(0.85), 
    ...
    plt.plot(df['wavelength'], df['absorbance'], label='Spectrum')
    plt.scatter(peak_wavelengths, peak_absorptions, color='red', label='Detected Peaks')
    plt.title(f"Spectrum Analysis: {drug_type}")
    plt.xlabel("Wavelength (nm)")
    plt.ylabel("Absorbance")
    plt.legend()
    plt.show()
    
    top_peaks = sorted(zip(peak_wavelengths, peak_absorptions), key=lambda x: x[1], reverse=True)[:5]
...
def augment_spectrum(df, n_augments=5):
    ...
    for _ in range(n_augments):
        ...
        # Random wavelength shift (small)
        shift = np.random.uniform(-1, 1)
        
        # Random intensity variation
        intensity = np.random.uniform(0.9, 1.1)
        
        new_df['wavelength'] = new_df['wavelength'] + shift
...
def extract_features(df):
    ...
    for drug, info in DRUG_INFO.items():
        ...
        for target_peak in info['key_peaks']:
            ...
            idx = (df['wavelength'] - target_peak).abs().idxmin()
            actual_peak = df['wavelength'].iloc[idx]
...
def prepare_training_data(drug_files, non_drug_files):
    ...
    for drug, path in drug_files.items():
        try:
            ...
            for _ in range(5):  # Multiple augmentations
                aug_df = augment_spectrum(df)
...
def predict_sample(file_path, binary_model, multiclass_model, le):
    ...
    try:
        ...
        if 'wavelength' not in df.columns or 'absorbance' not in df.columns:
            raise ValueError("Input CSV must contain 'wavelength' and 'absorbance' columns")
        ...
        try:
            ...
            print(f"⚠️ Couldn't get features from model, using fallback: {str(e)}")
...
if __name__ == "__main__":
    # Define input files
    drug_files = {
        'cocaine': "correct_cocaine.csv",
        'heroin': "correct_heroin.csv",
        'methadone': "correct_methadone.csv"
    }
    non_drug_files = ["correct_sucrose.csv"]
    ...
    try:
        ...
        test_files = {
            ...
            'heroin': "correct_heroin.csv",
            'methadone': "correct_methadone.csv",
            'sucrose': "correct_sucrose.csv"
        }
        
        print("\n🧪 Running Test Predictions:")
        file = "correct_sucrose.csv"
